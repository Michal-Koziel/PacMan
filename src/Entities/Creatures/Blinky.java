...
public class Blinky extends Creature{
	...
	public Blinky(Handler handler, float x, float y) {
		...
		state = "Chase";
		//Animation
		ChaseDown = new Animation(250,Assets.BlinkyDown);
		ChaseUp = new Animation(250,Assets.BlinkyUp);
		ChaseLeft = new Animation(250,Assets.BlinkyLeft);
		ChaseRight = new Animation(250,Assets.BlinkyRight);
	...
	public void update() {
		...
		if((counter == 300 && state == "Escape" )||( state == "Return" && counter == 120)) {
			SwitchState("Chase");
			counter = 0; 
			CounterOn = false;
		}
