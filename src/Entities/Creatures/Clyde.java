...
//Duch Clyde z okreœlonym prostok¹tem kolizji oraz przypisan¹ animacj¹
public class Clyde extends Creature{
	...
	public Clyde(Handler handler, float x, float y) {
		...
		state = "Chase";
		//Animation
		ChaseDown = new Animation(250,Assets.ClydeDown);
		ChaseUp = new Animation(250,Assets.ClydeUp);
		ChaseLeft = new Animation(250,Assets.ClydeLeft);
		ChaseRight = new Animation(250,Assets.ClydeRight);
	...
	// Wyliczenie ruchu goni¹cego Clyda
	private void GetChaseMoves() {
	   ...
	   // jeœli jest co najmniej 4 klocki od pacmana goni go jak blinky
		if(Utils.GetPath(playerX,playerY,(x+width/2+speed),(y+height/2)) >= 4*Tiles.TILEHEIGHT  ) {
		...
		} else {   // jesli jest blizej ucieka do prawego dolnego rogu mapy
			
			// zmiana targetu
			playerX = 18 * Tiles.TILEWIDTH;
			playerY = 18 * Tiles.TILEHEIGHT;
			// wykrycie kolizji na drodze w prawo,lewo,dó³,górê
			boolean path1,path2,path3,path4; 
			path1 = CollisionWithTile((int) (x+speed + bounds.x + bounds.width)/Tiles.TILEWIDTH, (int) (y+bounds.y)/Tiles.TILEHEIGHT) || 
					CollisionWithTile((int) (x+speed + bounds.x + bounds.width)/Tiles.TILEWIDTH, (int) (y+bounds.y + bounds.height)/Tiles.TILEHEIGHT);
